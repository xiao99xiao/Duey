...
struct RichTextEditorView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            RTFTextEditor(rtfData: Binding(
                get: { task.richContent },
                set: { task.richContent = $0 }
            ))
            .padding(12)
        }
    }
...
/// Text editor that works directly with RTF Data, bypassing AttributedString
/// to preserve NSTextAttachment (checkboxes)
struct RTFTextEditor: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack(alignment: .topLeading) {
                // Native Text Editor that saves/loads RTF directly
                RTFNativeTextView(rtfData: $rtfData, textViewRef: textViewRef)

                // Formatting Toolbar (appears when text is selected, positioned above or below selection)
                if textViewRef.hasSelection, textViewRef.selectionRect != .zero {
    ...
    private var isBold: Bool {
        guard let textView = textViewRef.textView,
              let font = textView.typingAttributes[.font] as? NSFont else { return false }
